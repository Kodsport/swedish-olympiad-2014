...
char getNth(Node* node, long long n, int howfar) {
	...
	if (howfar == 0) {
		...
		if (down.first <= n && n < down.first + down.second->size)
			return getNth(down.second, n - down.first, howfar - 1);
		else
			return getNth(node, n, howfar - 1);
	}
}
char getNthChar(Node* node, long long n) {
	return getNth(node, n, node->downwards.size());
}
