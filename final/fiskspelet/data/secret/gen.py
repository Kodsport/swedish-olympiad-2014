...
def transition_factor(size_from, size_to):
    ...
    if gets_smaller:
        ...
        return (difficulty >= difficulty_nomideat) + 1
...
fishes = []
x = 0
last_max_size = 2  # 2 == Stor
HEIGHT_OF_US = 7

while N > 0:
    max_size = lessen_ambition(x, last_max_size, random.randint(0, 2))
    xInc = int(x * (transition_factor(last_max_size, max_size) - 1))
    ...
    while y < h and N > 0:
        ...
        if ok:
            ...
            largest_outputted = max(largest_outputted, size)
        y += max(4 + tail_height(size) + 1, random.randint(0, h))
    last_max_size = largest_outputted
